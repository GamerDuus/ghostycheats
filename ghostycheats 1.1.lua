--- Menuen er et et remake af en allerede eksisterne menu dog lavet om til danske fucntioner
--- Pt er menuen undected af cocohax men det lort anyways
--- Kontakt os support@ghostycheats.com vis du finder fejl
--- Tak fordi i valgte og bruge os
--- alle rettigheder forbelholdes copyright @ Ghostycheats.com 2021
--- 
---

local appID = "296383927034052610"
local assetID = "hankgamesense"
...
Hankgamesense = NativeUI.CreateMenu("👻     ~r~Ghosty Cheats", "Stay ghosty", X, Y, "commonmenu", "gradient_bgd")

HankgamesensePool = NativeUI.CreatePool()
HankgamesensePool:Add(Hankgamesense)
...
do
    ...
    --==================================================================================================================================================--
    Custom = { --custom biler devo filerne
        "devo3", -- politibil
        "devo4", -- politibil
        "2019chiron",
        "599gtox",
...
WeaponComponentNames = {
    ...
    ["COMPONENT_KNUCKLE_VARMOD_gamesenseS"] = GetLabelText("WCT_KNUCK_BG"),
...
function SpawnWeaponMenu(WeaponSpawnMenu, PlayerPed)
    ...
    local handGunsMenu = (HankgamesensePool:AddSubMenu(WeaponSpawnMenu, "Håndpistoler"))
    handGunsMenu.Item:RightLabel(":🐔:")
    handGunsMenu = handGunsMenu.SubMenu

    local riflesMenu = (HankgamesensePool:AddSubMenu(WeaponSpawnMenu, "Assault Rifles"))
    riflesMenu.Item:RightLabel(":🐔:")
    riflesMenu = riflesMenu.SubMenu

    local shotgunsMenu = (HankgamesensePool:AddSubMenu(WeaponSpawnMenu, "Shotguns"))
    shotgunsMenu.Item:RightLabel("🐔")
    shotgunsMenu = shotgunsMenu.SubMenu

    local smgsMenu = (HankgamesensePool:AddSubMenu(WeaponSpawnMenu, "Sub-/Light Machine Guns"))
    smgsMenu.Item:RightLabel("🐔")
    smgsMenu = smgsMenu.SubMenu

    local throwablesMenu = (HankgamesensePool:AddSubMenu(WeaponSpawnMenu, "Throwables"))
    throwablesMenu.Item:RightLabel("🐔")
    throwablesMenu = throwablesMenu.SubMenu

    local meleeMenu = (HankgamesensePool:AddSubMenu(WeaponSpawnMenu, "Melee"))
    meleeMenu.Item:RightLabel("🐔")
    meleeMenu = meleeMenu.SubMenu

    local heavyMenu = (HankgamesensePool:AddSubMenu(WeaponSpawnMenu, "Heavy Weapons"))
    heavyMenu.Item:RightLabel("🐔")
    heavyMenu = heavyMenu.SubMenu

    local snipersMenu = (HankgamesensePool:AddSubMenu(WeaponSpawnMenu, "Sniper Rifles"))
...
do
    ...
    function doshit(playerVeh)
        ...
        SetVehicleNumberPlateText(playerVeh, "gamesenseFTW")
...
do
    ...
    Citizen.CreateThread(
        function()
            while ShowMenu do
                ...
                Hankgamesense.Logo:Colour(232, 233, 234, 255) --HeaderColor
            end
        end
    )
        ...
        function()
            while ShowMenu do
                ...
                if Enable_Mouse then
                    HankgamesensePool:MouseControlsEnabled(true)
                else
                    HankgamesensePool:MouseControlsEnabled(false)
                end
    ...
    local function AddPlayerOptionsMenu(menu)
        local PlayerOptionsMenu = (HankgamesensePool:AddSubMenu(menu, "Spiller Muligheder"))
        PlayerOptionsMenu.Item:RightLabel("")
        PlayerOptionsMenu = PlayerOptionsMenu.SubMenu


        local suicide = NativeUI.CreateItem("Selvmord", "    Dræber dig med det samme godt til at fake død")
        ...
        local revivebyid = NativeUI.CreateItem("genopliv (ESX)", "genopstå fra de døde")
        revivebyid.Activated = function(ParentMenu, SelectedItem)
            local id = FiveM.GetKeyboardInput("Indtast spiller id:")
        ...
        local heal = NativeUI.CreateItem("giv dig selv liv", "Få fuldt liv")
        heal.Activated = function(ParentMenu, SelectedItem)
            SetEntityHealth(PlayerPedId(), 200)
        end
        PlayerOptionsMenu:AddItem(heal)

        local armour = NativeUI.CreateItem("Giv skudsikkervest", "Giver dig fuldt armor")
        ...
        local setHunger = NativeUI.CreateItem("Fuld mad og vand (ESX)", "bliv mædt")
        ...
        local unlimitedStamina =
            NativeUI.CreateCheckboxItem("uendeligt Stamina", Enable_InfiniteStamina, "Løb løb løb.")
        ...
        local ExplosiveFists =
            NativeUI.CreateCheckboxItem("Explosive Fists", Enable_ExplosiveFists, "DAMM DEN SAD.")
        ...
        local CustomHeader = NativeUI.CreateCheckboxItem("Header", Enable_CustomHeader, "Virker måske?")
    ...
    local function AddOnlinePlayersMenu(menu)
        ...
        local OnlinePlayersMenu = (HankgamesensePool:AddSubMenu(menu, "Online Spiller"))
        OnlinePlayersMenu.Item:RightLabel("")

        local OnlinePlayerOptionsMenu = HankgamesensePool:AddSubMenu(OnlinePlayersMenu.SubMenu, "Online Spiller settings")

        do
            local Spectate =
                NativeUI.CreateItem("Spectate Spiller", "Spectate " .. PlayerName .. ". I'm Watching You X)")
            Spectate.Activated = function(ParentMenu, SelectedItem)
                SpectatePlayer(SelectedPlayer)
            end
            OnlinePlayerOptionsMenu.SubMenu:AddItem(Spectate)

            local Teleport = NativeUI.CreateItem("TP Til Spiller", "TP Til " .. PlayerName)
            Teleport.Activated = function(ParentMenu, SelectedItem)
                TeleportToPlayer(SelectedPlayer)
            end
            OnlinePlayerOptionsMenu.SubMenu:AddItem(Teleport)

            local TeleportToCar = NativeUI.CreateItem("TP ind i biler", "TP Til " .. PlayerName)
            TeleportToCar.Activated = function(ParentMenu, SelectedItem)
                local ped = GetPlayerPed(SelectedPlayer)
                if not IsPedInAnyVehicle(ped) then
                    return FiveM.Notify("Spiller er ikke i en bil")
                end
            ...
            local OpenInventory = NativeUI.CreateItem("Åben inventroy (ESX)", "sjæl folk anal dildo")
            ...
            local OpenInventoryThugLife = NativeUI.CreateItem("Open Inventory (ESX)", "v2 updated")
            ...
            local NativeSlay = NativeUI.CreateItem("Dræb", "Dræb denne person " .. PlayerName)
            ...
            local Fling = NativeUI.CreateItem("Fling Spilla", "Fly " .. PlayerName .. " God flyvetur")
            ...
            local Fake = NativeUI.CreateItem("falsk besked", "Hvorfor ikke " .. PlayerName)
            Fake.Activated = function(ParentMenu, SelectedItem)
                local messaggio = FiveM.GetKeyboardInput("Indtast besked max 100 ord", "", 100)
            ...
            local CagePlayer = NativeUI.CreateItem("Fang spiller", "Personen er da os bare en hund" .. PlayerName)
            ...
            local FIBPlayer = NativeUI.CreateItem("FIB Spiller", "FIB Building" .. PlayerName)
            ...
            local NativeExplode = NativeUI.CreateItem("lydløs Explode", "Explode " .. PlayerName .. " KABOOM")
    ...
    local function AddTeleportMenu(menu)
        local TeleportMenu = (HankgamesensePool:AddSubMenu(menu, "Teleportere Muligheder"))
    ...
    local function AddWeaponOptionsMenu(menu)
        local WeaponOptionsMenu = (HankgamesensePool:AddSubMenu(menu, "Weapon Options"))
        WeaponOptionsMenu.Item:RightLabel("")
        WeaponOptionsMenu = WeaponOptionsMenu.SubMenu

        local WeaponSpawnMenu = (HankgamesensePool:AddSubMenu(WeaponOptionsMenu, "Spawn Weapon"))
        ...
        local oneshot = NativeUI.CreateCheckboxItem("One Shot", Enable_OneShot, "Dræb folk med 1 skud")
        oneshot.CheckboxEvent = function(menu, item, enabled)
            Enable_OneShot = enabled
        end
        WeaponOptionsMenu:AddItem(oneshot)

        local RapidFire = NativeUI.CreateCheckboxItem("Rapid Fire", Enable_RapidFire, "ingen reload?")
        RapidFire.CheckboxEvent = function(menu, item, enabled)
            Enable_RapidFire = enabled
        end
        WeaponOptionsMenu:AddItem(RapidFire)

        local TriggerBot = NativeUI.CreateCheckboxItem("Trigger Bot", Enable_TriggerBot, "meget dårlig")
    ...
    local function AddVehicleAutoPilot(menu)
        local vehicleAutoPilotMenu =
            (HankgamesensePool:AddSubMenu(menu, "Vehicle Auto Pilot", "Manage vehicle auto pilot options."))
    ...
    local function AddVehicleOptionsMenu(menu)
        local VehicleOptionsMenu = (HankgamesensePool:AddSubMenu(menu, "Vehicle Options"))
        ...
        local repairVehicle =
            NativeUI.CreateItem("Fix bil", "Fixer din bil lige på stedet.")
        ...
        local repairEngine =
            NativeUI.CreateItem("Fix engine", "Fixer kun din engine.")
        ...
        local flipVehicle = NativeUI.CreateItem("Flip Din Bil", "Kør pænt :)")
        ...
        local waterp = NativeUI.CreateCheckboxItem("vandtæt bil", Enable_waterp, "Sneaky")
        ...
        local twostep = NativeUI.CreateCheckboxItem("2 Step", Enable_twostep, "Røvbanan")
        twostep.CheckboxEvent = function(menu, item, enabled)
            Enable_twostep = enabled
        end
        VehicleOptionsMenu:AddItem(twostep)

        local vehicleTypes = NativeUI.CreateListItem("Bil type", CarTypes, 1)
        VehicleOptionsMenu:AddItem(vehicleTypes)

        local spawnVehicle = NativeUI.CreateListItem("Spawn bil", CarsArray[1], 1)
        ...
        local spawnLegalVehicle = NativeUI.CreateItem("Spawn bil med navn", "Spawn en bil med navn.")
        spawnLegalVehicle.Activated = function(ParentMenu, SelectedItem)
            local ModelName = FiveM.GetKeyboardInput("Bilens navn", "", 10)
            local PlateNumber = FiveM.GetKeyboardInput("Nummerplade", "", 8)
        ...
        local refuelVehicle = NativeUI.CreateItem("fuld benzin (ESX)", "Giver bilen fuldt b enzin.")
        ...
        refuelVehicle.Activated = function(ParentMenu, SelectedItem)
            if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                ...
                FiveM.TriggerCustomEvent(
                    false,
                    "advancedFuel:setEssence", ---> Dette er esx vej til at genopfylde benzin
                    100,
                ...
                FiveM.Notify("Du skal være i en bil for at gøre dette !", NotificationType.Error)
            end
        end
        VehicleOptionsMenu:AddItem(refuelVehicle)
        FiveM.SetResourceLocked("advancedFuel", refuelVehicle)

        local maxTuneVehicle = NativeUI.CreateItem("Max Tune", "Max tune your vehicle.") --- Skal have en update
        maxTuneVehicle.Activated = function(ParentMenu, SelectedItem)
            if IsPedInAnyVehicle(GetPlayerPed(-1)) then
        ...
        local sellVehicle = NativeUI.CreateItem("Sælg Bil (ESX)", "kun esx.")
        sellVehicle.Activated = function(ParentMenu, SelectedItem)
            if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                if ESX ~= nil then
                    ...
                    FiveM.Notify("FEJL SCRIPT IKKE FUNDET!", NotificationType.Error)
                end
            else
                FiveM.Notify("Du skal sidde i en bil for at gøre dette !", NotificationType.Error)
            end
        end
        VehicleOptionsMenu:AddItem(sellVehicle)
        FiveM.SetResourceLocked("esx_vehicleshop", sellVehicle)

        local stealVehicle = NativeUI.CreateItem("Stjæl bil (ESX)", "Gøre den bil du sidder i til din egen.")
        stealVehicle.Activated = function(ParentMenu, SelectedItem)
            if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                ...
                if SpawnedVehicleProperties then
                    ...
                    FiveM.Notify("~h~Du gjorde det!", NotificationType.Success)
                end
            else
                FiveM.Notify("Du skal være i en bil !", NotificationType.Error)
            end
        end
        VehicleOptionsMenu:AddItem(stealVehicle)
        FiveM.SetResourceLocked("esx_vehicleshop", stealVehicle)

        local deleteVehicle = NativeUI.CreateItem("Delete Vehicle (~g~Updateing~w~)", "Delete your vehicle.")
        ...
        local dirtyVehicle = NativeUI.CreateItem("Dirty Vehicle (~r~Alle servere~w~)", "Make your vehicle dirty.")
        ...
        local cleanVehicle = NativeUI.CreateItem("Clean Vehicle (~r~Alle servere~w~)", "Make your vehicle clean.")
        ...
        local DriftVehicle = NativeUI.CreateItem("Tokyo Drift (~r~Alle servere~w~)", "Drift Like Han.")
        ...
        local SuperHandling = NativeUI.CreateItem("Super Handle (~r~Alle servere~w~)", "Batmobile Handling.")
        ...
        local changeVehiclePlate =
            NativeUI.CreateItem("Custom nummerplade", "Lav en custom nummerplade.")
        changeVehiclePlate.Activated = function(ParentMenu, SelectedItem)
            if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                local playerVeh = GetVehiclePedIsIn(GetPlayerPed(-1), true)
                local result = FiveM.GetKeyboardInput("Indtast din ønskede nummerplade", "", 8)
                if result then
                    SetVehicleNumberPlateText(playerVeh, result)
                end
            else
        ...
        local ESXDrivingLicense = NativeUI.CreateItem("Kørekort (~r~ESX~w~)", "Så kan du køre lolvligt")
        ...
        local VRPDrivingLicense = NativeUI.CreateItem("Kørekort (~r~vrp~w~)", "Så kan du køre lovligt")
    ...
    local function AddHarvestMenu(menu)
        local SettingOptionsMenu = (HankgamesensePool:AddSubMenu(menu, "Triggers"))
        SettingOptionsMenu.Item:RightLabel("~r~")
        ...
        local MoneyWash3 = NativeUI.CreateItem("~r~Luk menu", "Bemærk! menuen køre i baggrunden")
    ...
    local function TrollOptionsMenu(menu)
        local TrollOptionsMenu = (HankgamesensePool:AddSubMenu(menu, "Troll Options"))
        ...
        local RampTheCar = NativeUI.CreateItem("FIB CARS", "Dette er eer en ny function. ")
        ...
        local RampTheCar2 = NativeUI.CreateItem("Ramp Cars", "Dette er en ny funtion. ")
        ...
        local NPlate = NativeUI.CreateItem("Skift nummerplade", "Nummerpladeskift")
        NPlate.Activated = function(ParentMenu, SelectedItem)
            local plateInput = FiveM.GetKeyboardInput("Indtast nummerplade (max 8 bogstaver:)", "", 8)
            for k in EVehicles() do
                RequestControlOnce(k)
        ...
        FakeM.Activated = function(ParentMenu, SelectedItem)
            local pname = FiveM.GetKeyboardInput("Indtast spiller navn max 100 ord", "", 100)
            if pname then
                local message = FiveM.GetKeyboardInput("Îndtast besked max 1000 ord", "", 1000)
                if message then
        ...
        local DisCars = NativeUI.CreateCheckboxItem("Deaktiver Cars", Enable_DisCars, "Slår biler fra")
        DisCars.CheckboxEvent = function(menu, item, enabled)
            Enable_DisCars = enabled
        end
        TrollOptionsMenu:AddItem(DisCars)

        local DisGuns = NativeUI.CreateCheckboxItem("Deaktiver våben", Enable_DisGuns, "Slår våben fra")
        DisGuns.CheckboxEvent = function(menu, item, enabled)
            Enable_DisGuns = enabled
        end
        TrollOptionsMenu:AddItem(DisGuns)

        local bombs = NativeUI.CreateCheckboxItem("Bombe det Top Shootas", Enable_BombShooters, "Bang bang")
        bombs.CheckboxEvent = function(menu, item, enabled)
            Enable_BombShooters = enabled
        end
        TrollOptionsMenu:AddItem(bombs)

        local FTW = NativeUI.CreateCheckboxItem("Fuck Verden op", Enable_WorldFuck, "LOL")
        FTW.CheckboxEvent = function(menu, item, enabled)
            Enable_WorldFuck = enabled
        end
        TrollOptionsMenu:AddItem(FTW)

        local Crash = NativeUI.CreateCheckboxItem("Crash Server", Enable_ServerCrash, "UPDATEING")
        ...
        local GcPhone =
            NativeUI.CreateCheckboxItem("GcPhone Crasher", Enable_GcPhone, "Du lagger selv :)")
        GcPhone.CheckboxEvent = function(menu, item, enabled)
            Enable_GcPhone = enabled
        end
        TrollOptionsMenu:AddItem(GcPhone)

        local Jail = NativeUI.CreateCheckboxItem("Fængsel server", Enable_Jail, "Nice one")
    ...
    local function AddWorldOptionsMenu(menu)
        local WorldOptionsMenu = (HankgamesensePool:AddSubMenu(menu, "ESP Muligheder"))
        WorldOptionsMenu.Item:RightLabel("")
        WorldOptionsMenu = WorldOptionsMenu.SubMenu

        local ESPOptionsMenu = (HankgamesensePool:AddSubMenu(WorldOptionsMenu, "ESP + MORE"))
        ...
        do
            ...
            local espInfo =
                NativeUI.CreateCheckboxItem("Spiller Info", ShowEspInfo, "Display Spiller-Info på deres hoveder.")
            ...
            local espOutline =
                NativeUI.CreateCheckboxItem("Spiller Boxes", ShowEspOutline, "Display og outline around players.")
            ...
            local espLines =
                NativeUI.CreateCheckboxItem("Spiller Linjer", ShowEspLines, "Display lines connecting to all players.")
            ...
            local espHeadSprites =
                NativeUI.CreateCheckboxItem("Spiller navn", ShowHeadSprites, "Display Player Names on their heads.")
            espHeadSprites.CheckboxEvent = function(menu, item, enabled)
                ShowHeadSprites = enabled
            end
            ESPOptionsMenu:AddItem(espHeadSprites)

            local heatVision = NativeUI.CreateCheckboxItem("Varme Vision", enableThermalVision, "Enable Thermal Vision")
            heatVision.CheckboxEvent = function(menu, item, enabled)
                SetSeethrough(enabled)
            end
            ESPOptionsMenu:AddItem(heatVision)

            local nightVision = NativeUI.CreateCheckboxItem("Nat Vision", enableNightVision, "Enable Thermal Vision")
        ...
        local playerBlips = NativeUI.CreateCheckboxItem("Spiller Blips", ShowPlayerBlips, "Viser spillere på mappet")
        playerBlips.CheckboxEvent = function(menu, item, enabled)
            ShowPlayerBlips = enabled
        end
        WorldOptionsMenu:AddItem(playerBlips)

        local crosshair = NativeUI.CreateCheckboxItem("Crosshair", ShowCrosshair, "Kommer an på server")
        crosshair.CheckboxEvent = function(menu, item, enabled)
            ShowCrosshair = enabled
        end
        WorldOptionsMenu:AddItem(crosshair)

        local crosshair2 = NativeUI.CreateCheckboxItem("Custom Crosshair", ShowCrosshair1, "Dette virker altid ;)")
    ...
    local function AddIPMenu(menu)
        local IPMenu = (HankgamesensePool:AddSubMenu(menu, GetCurrentServerEndpoint()))
        IPMenu.Item:RightLabel("🐔")
    end

    AddPlayerOptionsMenu(Hankgamesense)
    AddOnlinePlayersMenu(Hankgamesense)
    AddTeleportMenu(Hankgamesense)
    AddWeaponOptionsMenu(Hankgamesense)
    AddVehicleOptionsMenu(Hankgamesense)
    AddHarvestMenu(Hankgamesense)
    TrollOptionsMenu(Hankgamesense)
    AddWorldOptionsMenu(Hankgamesense)
    AddSuicideMenu(Hankgamesense)
    AddIPMenu(Hankgamesense)
end

HankgamesensePool:MouseControlsEnabled(false)
HankgamesensePool:MouseEdgeEnabled(false)
HankgamesensePool:ControlDisablingEnabled(false)

HankgamesensePool:RefreshIndex()

Citizen.CreateThread(
    function()
        while ShowMenu do
            HankgamesensePool:ProcessMenus()
            if IsDisabledControlJustPressed(0, 121) then
                HankgamesensePool:RefreshIndex()
                if HankgamesensePool:IsAnyMenuOpen() then
                    HankgamesensePool:CloseAllMenus()
                else
                    Hankgamesense:Visible(true)
                end
            end
            Citizen.Wait(0)
        end
